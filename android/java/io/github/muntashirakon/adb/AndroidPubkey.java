...
final class AndroidPubkey {
    ...
    public static byte[] adbAuthSign(PrivateKey privateKey, byte[] payload)
     ...
     */
    public static byte[] encodeWithName(RSAPublicKey publicKey, String name)
        ...
        try (ByteArrayNoThrowOutputStream bos = new ByteArrayNoThrowOutputStream(pkeySize + name.length() + 2)) {
            bos.write(Base64.encode(encode(publicKey), Base64.NO_WRAP));
    ...
    static byte[] getUserInfo(String name) {
     ...
     */
    public static RSAPublicKey decode(byte[] androidPubkey)
     ...
     * {@link #ANDROID_PUBKEY_ENCODED_SIZE}
     */
    public static byte[] encode(RSAPublicKey publicKey) throws InvalidKeyException {
        BigInteger r32;
        BigInteger n0inv;
        BigInteger rr;
    ...
    static boolean fitsInBytes(byte[] bytes, int num_bytes, int len) {
        byte mask = 0;
