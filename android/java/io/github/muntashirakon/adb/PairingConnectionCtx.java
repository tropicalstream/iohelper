...
public final class PairingConnectionCtx implements Closeable {
    ...
    public PairingConnectionCtx(String host, int port, byte[] pswd, KeyPair keyPair,
                                String deviceName)
    ...
    public PairingConnectionCtx(String host, int port, byte[] pswd, PrivateKey privateKey,
                                Certificate certificate, String deviceName)
            throws NoSuchAlgorithmException, KeyManagementException, InvalidKeyException {
    ...
    @Override
    public void close() {
        // Null-safe on every field: this runs from try-with-resources, so it
        // also runs when setupTlsConnection() threw before any of them existed.
        // An NPE here would replace the real reason pairing failed - usually
        // "the pairing dialog was closed" - with a stack trace about streams.
