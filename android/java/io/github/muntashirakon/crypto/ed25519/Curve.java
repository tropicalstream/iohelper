...
public class Curve implements Serializable {
    ...
    /**
     * Decode a point. NOTE: despite the inherited name this does NOT negate -
     * the sign test below is {@code !=}, so the point returned is the one that
     * was encoded, matching BoringSSL's x25519_ge_frombytes_vartime rather than
     * ref10's ge_frombytes_negate_vartime. SPAKE2 needs exactly that; nothing
     * else in this tree calls it.
     *
     * @param s The 32-byte encoded point.
     * @return The decoded point, or null if s is not on the curve.
     */
    public GroupElement fromBytesNegateVarTime(final byte[] s) {
        ...
        // The sign bit, and only the sign bit. s[31] is a signed byte, so the
        // obvious (s[31] >>> 7) sign-extends every encoding whose top bit is
        // set into 0x01ffffff - never equal to 0 or 1, so the branch was taken
        // unconditionally and half of all points decoded to their negation.
        int isNegative = X.isNegative() ? 1 : 0;
        if (isNegative != ((s[31] >> 7) & 1)) {
